...
**使い分けの例:**
- セクター内で敵に近づく → ワープ `0.125` ～ `0.5`
- 隣のクオドラントへ移動 → ワープ `1`
- 遠くのクオドラントへ急行 → ワープ `2` ～ `8`
...
**ダメージの発生:**
- 被弾時に60%の確率でランダムなシステムが損傷
- ダメージ量: `-0.5` ～ `-3.0`（累積）

**修理速度:**

| 状態 | 修理レート | 例: damage = -3.0 の場合 |
|------|-----------|------------------------|
| ドッキング中 | 1.0 | 3日で修理完了 |
| 航行中 | 0.1 | 30日で修理完了 |
...
**修理計算:**
```python
damage += repair_rate × time_elapsed
# damageが0以上になったら修理完了
```

**ポイント:**
- DAMコマンドで表示される数値（例: `-2.5`）は修理完了までの残り日数
- ドッキング中は10倍速で修理される
- ダメージは累積するので、連続被弾すると修理時間が増える
